...
export class QrError extends Error {
  kind: 'image' | 'capacity' | 'render';

  constructor(kind: QrError['kind'], message: string) {
    super(message);
    this.kind = kind;
  }
}
...
export function readImageFile(file: File) {
  ...
  if (!types.includes(file.type)) {
    return Promise.reject(
      new QrError('image', 'Choose a PNG, JPG, WebP, GIF, or SVG image.'),
    );
  }
  if (file.size > 5 * 1024 * 1024) {
    return Promise.reject(
      new QrError('image', 'Choose an image smaller than 5 MB.'),
    );
  }
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = () =>
      reject(
        new QrError('image', 'This image could not be read. Try another file.'),
      );
    reader.onload = () => {
...
export async function renderQrCode(
  ...
  try {
    ...
    throw new QrError(
      'capacity',
      'This URL is too long for a QR code. Try a shorter link.',
    );
  }
  ...
  if (!context)
    throw new QrError(
      'render',
      'The QR preview could not be drawn. Try reloading.',
    );
